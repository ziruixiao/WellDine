...
+ (void)dismiss
{
	if ([sharedDialog respondsToSelector:@selector(presentingViewController)])
		[[sharedDialog presentingViewController] dismissViewControllerAnimated:YES completion:nil];//CHANGED
	else 
		[[sharedDialog parentViewController] dismissViewControllerAnimated:YES completion:nil];//CHANGED
}
...
- (void)dismiss
{
	if (self == sharedDialog) {
		...
		if ([self respondsToSelector:@selector(presentingViewController)])
			[[self presentingViewController] dismissViewControllerAnimated:YES completion:nil];//CHANGED
		else
			[[self parentViewController] dismissViewControllerAnimated:YES completion:nil];//CHANGED
	}
}
...
- (void)show
{
	...
	[[self presentingController] presentViewController:self animated:YES completion:nil]; //CHANGED
}
