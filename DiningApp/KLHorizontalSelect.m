...
-(void) scrollViewDidFinishScrolling: (UIScrollView*) scrollView {
    ...
    if ([self.tableView indexPathForRowAtPoint:point].row != 1) {
        //Hide the arrow when scrolling
        [self setCurrentIndex:[self.tableView indexPathForRowAtPoint:point]];
    }
    [self.arrow show:YES];
    
}
-(void) setCurrentIndex:(NSIndexPath *)currentIndex2 {
